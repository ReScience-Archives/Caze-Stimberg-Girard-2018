...
\textbf{Qualitative network behaviour as a function of excitatory and inhibitory connection strength for linearly (A) and non-linearly (B, C) coupled networks.} All conventions as in Fig.~\ref{fig:grid}, but points are coloured according the observed qualitative behaviour that occurred the most often among the 10 independent simulations for given connection strengths.}
\end{figure}
...
\textbf{Evolution of synchronous pulses in linearly (A) and non-linearly (B) coupled networks, corrected for the background activity.} All conventions as in Fig.~\ref{fig:markov}. Without correction (see text)}
\end{figure}
\end{document}
